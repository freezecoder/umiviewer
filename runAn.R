...
plotMetrics = function(ExpName, Species = 1, PCT_MRNA = .4, NUM_Transcripts = 50){
 ...
 png(imgfile,width = 540, height = 600)
 par(mfrow=c(2,3))
  if (Species == 1) { 
    ...
    plot(x$Human_Percentage,x$Mouse_Percentage,xlab="Human Percentage",ylab="Mouse Percentage")
    plot(x$Human_Genes,x$Mouse_Genes,xlab="Human Genes",ylab="Mouse Genes")
  }
 ...
 title(ExpName, outer=FALSE)
 dev.off()
 print("Done, Wrote Species Identity file")
 print(imgfile)
}
...
for (file in args) {
  ifile=file
  ifile=gsub(".umi.txt|.metrics.txt","",ifile)
  print(ifile)
  plotMetrics(ifile)  
}
